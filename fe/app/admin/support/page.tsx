...
import { ChevronLeft, MessageSquare, CheckCircle2, AlertCircle, Send } from "lucide-react"
import { mockUsers } from "@/lib/mock-data"

export default function AdminSupportPage() {
  const { user } = useAuth()
  const { supportTickets, updateTicketStatus, addResponseToTicket } = useMockData()
  ...
  const getCustomerName = (userId: string) => {
    const customer = mockUsers.find((u) => u.id === userId)
  ...
  const handleAddResponse = () => {
    if (selectedTicketId && responseText.trim()) {
      addResponseToTicket(selectedTicketId, responseText)
      setResponseText("")
    }
  }
                  ...
                  {supportTickets.map((ticket) => (
                    ...
                    >
                      <td className="px-4 py-3">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">{getCustomerName(ticket.userId)}</p>
                        <span
            ...
            <Card className="lg:col-span-2 p-6 flex flex-col">
              <div className="flex justify-between items-start mb-4 pb-4 border-b">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">From: {getCustomerName(selectedTicket.userId)}</p>
                  <p className="text-sm text-muted-foreground">{selectedTicket.createdAt.toLocaleDateString()}</p>
                </div>
                <Select
                  value={selectedTicket.status}
                  onValueChange={(newStatus) =>
                    updateTicketStatus(selectedTicket.id, newStatus as "open" | "in-progress" | "closed")
                  }
                >
